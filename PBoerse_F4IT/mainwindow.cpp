// 202108231415  Postleizahlen bei Ortsangabe
// reset der Eingabefelder bei Startseite
// 202108241200  Ausgabe der Tabellenanzeige in csv Datei
    ...
    , ui(new Ui::MainWindow)
{
    ...
    if (home.isSslSupported()) { // true/false
           home.request();
           qDebug()<<__FILE__<<": "<<__LINE__<<home.getCoords();
 ...
 // 1)-------------------DB-QDEBUG----------------------------------
    qDebug()<<__FILE__<<": "<<__LINE__<<test.connect("fLo9o8_8");
    ...
    //qDebug()<<__FILE__<<": "<<__LINE__<<"enumcount"<<test.get_enumListe("Mitarbeiter").count();
    for(int i =0; i <test.get_enumListe("Mitarbeiter").count(); i++)
        //qDebug()<<__FILE__<<": "<<__LINE__<<"enum"<<test.get_enumListe("Mitarbeiter").at(i);
...
void MainWindow::on_pushStartseite_clicked()
{
    ...
    MainWindow::on_firmaStandortReset_clicked();
...
void MainWindow::on_pushImportManuell_clicked()
{
      ...
      ui->firmaName->setStyleSheet("border: 1px solid red");
      ui->firmaRechtsform->setStyleSheet("border: 1px solid red");
      ui->standortPlz->setStyleSheet("border: 1px solid red");
      ui->standortStrasse->setStyleSheet("border: 1px solid red");
...
void MainWindow::on_firmaStandortAnlegen_clicked()
{
    ...
    qDebug()<<__FILE__<<": "<<__LINE__<< "firmaMap:";
        qDebug()<<__FILE__<<": "<<__LINE__<< firmaMap.keys();
        qDebug()<<__FILE__<<": "<<__LINE__<< firmaMap.values()<<firmaMapBool;
        test.execQuery(firmaMap, firmaMapBool);
        ui->firmaStandortAnlegen->setEnabled(false);
...
void MainWindow::on_firmaStandortReset_clicked()
{
    ...
    ui->standortMitarbeiter->setCurrentIndex(0);
...
void MainWindow::on_firmaName_editingFinished()
{
    ...
    //qDebug()<<__FILE__<<": "<<__LINE__<< test.execQuery("SELECT name, rechtsform, website, branche, Personalvermittler, inhouse_IT FROM firma WHERE name = '"+ ui->firmaName->text()+"'");
    ...
    if (ui->firmaName->text() == test.get_query().value(1).toString())
    {
        ...
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true, "firma",ui->firmaName));
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true, "recht",ui->firmaRechtsform));
    }
    else
        firmaMapBool = false;
...
void MainWindow::on_standortPlz_editingFinished()
{
    ...
    //qDebug()<<__FILE__<<": "<<__LINE__<< rc << "+" << ui->standortPlz->text();
    bool vorh = false;
    for (int i = 0; i<rc ;i++)
    {
    ...
    ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(vorh, "vorhplz",ui->standortPlz));
...
void MainWindow::on_standortStrasse_editingFinished()
{
    //QMessageBox::information(this, Q_FUNC_INFO,"Ab Zeile " + QString::number(__LINE__));
    int rc = ui->tableViewTrefferTabelleAnzeige->model()->rowCount();
    //qDebug()<<__FILE__<<": "<<__LINE__<< rc << "+" << ui->standortStrasse->text();
    bool vorh = false;
    for (int i = 0; i<rc ;i++)
    {
    ...
    ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(vorh, "vorhstra",ui->standortStrasse));

        //qDebug()<<__FILE__<<": "<<__LINE__<< "formstr: " <<formular::formstr;
...
void MainWindow::on_standortOrt_editingFinished()
{
    ...
    ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true,"stplz",ui->standortPlz));
}
...
{
   // QMessageBox::information(this, Q_FUNC_INFO,"Ab Zeile " + QString::number(__LINE__));
    qDebug()<<__FILE__<<" "<<__LINE__<<"Open "<<ui->lineEditCSVimport->text();
    FileHandling input(ui->lineEditCSVimport->text());

   // input.readCSVInto(ui->tableViewTrefferTabelleAnzeige);

    if(input.open(QIODevice::ReadOnly  | QIODevice::Text))
    {        // befüllen der Tabelle strassen mit den Feldern
        QStringList felderTabelle ={"ort", "strasse"," plz"};
        int colOfTable = 0;

        qDebug()<<__FILE__<<" "<<__LINE__<<"File is Open "<<ui->lineEditCSVimport->text();
        QTextStream stream(&input);
        int zeilenNummer = 0;
        for (QString line = stream.readLine();
             !line.isNull();
             line = stream.readLine()) {
            /* process line with information */
            zeilenNummer++;
            qDebug()<<__FILE__<<" "<<__LINE__<<" Zeile gelesen "<<zeilenNummer<<":"<<line;
            QString field ="";
            for(QChar c : line )
            { //qDebug()<<__FILE__<<" "<<__LINE__<<":"<<c;
                if(c!=',')
                {
                   field += c; // Zeichen gehört noch zum Wort
                }
                else
                {   // Wort ist beendet, da Trenner erkannt wurde
                    qDebug()<<__FILE__<<" "<<__LINE__<<colOfTable<<".Spalte";
                    if(colOfTable < felderTabelle.length())
                    {qDebug()<<__FILE__<<" "<<__LINE__<<felderTabelle.at(colOfTable)<<":"<<field;
                    field ="";
                    colOfTable++;
                    }
                    else
                    {
                        qDebug()<<__FILE__<<" "<<__LINE__<<"Fehlerhafte Zeile "<<zeilenNummer<<":"<<line;
                    }
                }
            } // Nach Ende der Zeile
            if(colOfTable == felderTabelle.length()-1)
            {
            qDebug()<<__FILE__<<" "<<__LINE__<<colOfTable<<".Spalte";
            qDebug()<<__FILE__<<" "<<__LINE__<<felderTabelle.at(colOfTable)<<":"<<field;
    ...
    {
        QMessageBox::information(this, Q_FUNC_INFO,"File open failed " + QString::number(__LINE__));
    }
...
{
    QString file_name = QFileDialog::getOpenFileName(this, "open a file", "D:/_dmnbcFirma/Projekte/Fachmodul1/versionAb0823/Firmen4IT-main/documents", tr("csv Files(*.csv)"));
...
void MainWindow::Standorte_aus_Tabelle_anzeigen()
{
    ...
    qDebug()<<__FILE__<<": "<<__LINE__<<"Übergabe Daten "<<__FILE__<<__LINE__;
    qDebug()<<__FILE__<<": "<<__LINE__<<latList;
    qDebug()<<__FILE__<<": "<<__LINE__<<lngList;
    qDebug()<<__FILE__<<": "<<__LINE__<<firmList;
...
void MainWindow::on_firmaName_textEdited(const QString &arg1)
{
    if (ui->firmaName->text().length()==1)
    {
        ...
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true, "firma",ui->firmaName));
        ui->firmaName->setStyleSheet("border: 1px solid black");
    }
    if (ui->firmaName->text().length()==0)
    {
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(false, "firma",ui->firmaName));
        ui->firmaName->setStyleSheet("border: 1px solid red");
    }
}
...
void MainWindow::on_firmaRechtsform_textEdited(const QString &arg1)
{
    if (ui->firmaRechtsform->text().length()==1)
    {
        ...
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true, "recht",ui->firmaRechtsform));
        ui->firmaRechtsform->setStyleSheet("border: 1px solid black");
    }
    if (ui->firmaRechtsform->text().length()==0)
       {
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(false, "recht",ui->firmaRechtsform));
        ui->firmaRechtsform->setStyleSheet("border: 1px solid red");
    }
}
...
void MainWindow::on_standortPlz_textEdited(const QString &arg1)
{
    if (ui->standortPlz->text().length()==1)
    {
        ...
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true, "stplz",ui->standortPlz));
        ui->standortPlz->setStyleSheet("border: 1px solid black");

    }
    if (ui->standortPlz->text().length()==0)
    {
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(false, "stplz",ui->standortPlz));
        ui->standortPlz->setStyleSheet("border: 1px solid red");
    }
}
...
void MainWindow::on_standortStrasse_textEdited(const QString &arg1)
{
    if (ui->standortStrasse->text().length()==1)
       {
        ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(true, "ststr",ui->standortStrasse));
        ui->standortStrasse->setStyleSheet("border: 1px solid black");
       }

    if (ui->standortStrasse->text().length()==0)
       {
         ui->firmaStandortAnlegen->setEnabled(formular::pflicht_gefuellt(false, "ststr",ui->standortStrasse));
         ui->standortStrasse->setStyleSheet("border: 1px solid red");
        }

}

void MainWindow::on_pushWriteFile_clicked()
{
    FileHandling csvFile(QFileDialog::getSaveFileName(this,"Dateiname",QDir::currentPath(),"CSV Files (*.csv)"));
    // csvFile.writeCSV(test.get_query());
    csvFile.writeCSV(ui->tableViewTrefferTabelleAnzeige);
}
