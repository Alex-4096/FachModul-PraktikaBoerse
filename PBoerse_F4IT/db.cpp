// 202108241530  Eintragen neuer Standort in DB, neue Firma
#include "db.h"
#include "helper.h"
#include <QDebug>
#include <QString>
...
bool DB::execQuery(QMap<QString, QString> firmen_daten, bool exist)
{
    ...
    if(exist) // nur Standort eintragen
        {
           QString cmd = "SELECT Firma_ID from firma WHERE Name = '" + firmen_daten["firmaName"] + "'";
            bool ok = query.exec(cmd);

            qDebug( )<<__FILE__<<" "<<__LINE__<<"cmd : "<<cmd<<" ok: "<<ok;

            query.first();
            //to do   sql string gegen leerzeichen abfedern !!!
            QString firma_id = query.value(0).toString();
            qDebug()<<__FILE__<<" "<<__LINE__<<"firma_id"<<firma_id;
            cmd = "INSERT INTO standort (Firma_ID, Zentrale,PLZ,Strasse, ORT, Mitarbeiter,TelefonStandort) VALUES("
                           + firma_id + ","
    ...
    else // erst Firma, dann ersten Standort
        {
             QString cmd = "INSERT INTO firma (Name, Rechtsform, Branche, PLZ_Zentrale, ORT_Zentrale, Website, Inhouse_IT,Personalvermittler) VALUES('"
             ...
             QString firma_id = query.lastInsertId().toString();
...
array<double, 2> DB::get_coordinate(int firma_id, QString plz)
{
    ...
    qDebug()<<__FILE__<<": "<<__LINE__<<query.exec(cmd);
    qDebug()<<__FILE__<<": "<<__LINE__<<"Return Zeilen-Anzahl: "<<query.numRowsAffected();
    //---------------------------------------------//
    ...
    while(query.next())
    {
        ...
        qDebug()<<__FILE__<<": "<<__LINE__<<"Abfrage-Ausgabe:\t"<<abfrage_erg;
    }
    //---------------------------------------------//
    ...
    {
        qDebug()<<__FILE__<<": "<<__LINE__<<"Koordinaten in Array fuellen";
        while(query.next())
        {
        ...
        qDebug()<<__FILE__<<": "<<Q_FUNC_INFO<<__LINE__<<"Fehler beim Befüllen des Array -> passen nur zwei Werte";
    //---------------------------------------------//

    // Ausgabe der Koordinaten aus dem Array im Debug
    QVariant coord_groesse;
    qDebug()<<__FILE__<<": "<<__LINE__<<"Anzahl der Werte im Array: "<<coordinate.size();
    for(int i = 0; i < coord_groesse.fromValue(coordinate.size()).toInt(); i++)
    {
        qDebug()<<__FILE__<<": "<<__LINE__<<"Nummer: "<<i+1;
        qDebug()<<__FILE__<<": "<<__LINE__<<coordinate.at(i);
    }

    qDebug()<<__FILE__<<": "<<Q_FUNC_INFO<<"\tENDE\n"<<__FILE__;
    return coordinate;
}
...
QList<QString> DB::get_enumListe(QString enum_spalte)
{
    ...
    // Abfrage ausführen
    query.exec(cmd);
    qDebug()<<__FILE__<<": "<<__LINE__<<"Ausgefuehrte Abfrage:\n"<<query.lastQuery();
    //---------------------------------------------//
    ...
    qDebug()<<__FILE__<<": "<<__LINE__<<"Test-Ausgabe der QList:";
...
QList<QString> DB::get_pflichtfeld(QString tabelle)
{
    ...
    qDebug()<<__FILE__<<": "<<__LINE__<<"ausgefuehrte Abfrgae:\n"<<query.lastQuery();
    qDebug()<<__FILE__<<": "<<__LINE__<<"Result-Rows: "<<query.numRowsAffected();
    //---------------------------------------------//
    ...
    if(query.numRowsAffected() > 0)
    {
        ...
        // Test-Ausgabe der QList im Debug
        qDebug()<<__FILE__<<": "<<__LINE__<<"Größe von QList: "<<pflichtfeld.size();
        for(int i = 0; i < QVariant::fromValue(pflichtfeld.size()).toInt(); i++)
        {
            qDebug()<<__FILE__<<": "<<__LINE__<<"Pflichtfeld: "<<pflichtfeld.value(i);
        }
    }
    else
    {
        qDebug()<<__FILE__<<": "<<__LINE__<<"QList-Größe: "<<pflichtfeld.size();
        qDebug()<<__FILE__<<": "<<__LINE__<<"Tabelle "+tabelle+" hat keine Pflichtfelder";
    }
    //---------------------------------------------//
